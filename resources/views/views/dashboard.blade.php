...
<div class="exam-study-container">
    ...
    @php
        $isCompleted = $mapel_data["status_dikerjakan"] === "telah dikerjakan";
    @endphp
    <div class="exam-list {{ $isCompleted ? 'exam-completed' : '' }}">
        ...
        <div class="exam-list-go">
            @if (!$isCompleted)
            <a href="/kerjakan/{{ $mapel_data['kerjakan_link'] }}-{{ $mapel_data['penugasan_id'] }}/" class="exam-list-go-confirmation">lihat</a>
            @endif
        </div>
    </div>
    @endforeach
...
<script>
    ...
    $.ajax({
        ...
        success: function (result) {
            $("#alert-sambutan").html("Selamat datang " + result.data + " di UjianOnline!")
        }
    })
</script>
@endsection('script')
